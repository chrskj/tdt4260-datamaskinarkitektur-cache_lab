...
void avdc_access(avdark_cache_t *self, avdc_pa_t pa, avdc_access_type_t type)
{
    ...
    int hit;
    search_set(self, tag, &hit, &index, &first_free_index);

    if (!hit) {
        if (first_free_index == -1) {
            find_lru_index(self, &index);
        }
        else {
    ...
    self->lines[index].timestamp = std::clock();
    ...
    //avdc_print_internals(self);
}

void search_set(avdark_cache_t *self, avdc_tag_t tag, int* hit_ptr, int* index_ptr,
                int* first_free_index_ptr)
{
    for (int i = 0; i < (int)self->assoc; i++) {
        *hit_ptr = self->lines[*index_ptr+i].valid && self->lines[*index_ptr+i].tag == tag;
        if (*hit_ptr) {
            *index_ptr += i;
            return;
        }
        if (*first_free_index_ptr == -1 && !self->lines[*index_ptr+i].valid) {
            *first_free_index_ptr = *index_ptr + i;
        }
    }
}

void find_lru_index(avdark_cache_t *self, int* index_ptr) 
{
    std::clock_t curr_timestamp = self->lines[*index_ptr].timestamp;
    int curr_index = *index_ptr;
    for (int i = 1; i < (int)self->assoc; i++) {
        std::clock_t next_timestamp = self->lines[*index_ptr+i].timestamp;
        if (next_timestamp < curr_timestamp) {
            curr_timestamp = next_timestamp;
            curr_index = *index_ptr+i;
        }
    }
...
{
    /* TODO: Update this function */
    for (int i = 0; i < (int)(self->number_of_sets*self->assoc); i++) {
        self->lines[i].valid = 0;
        self->lines[i].tag = 0;
        self->lines[i].timestamp = std::clock();
    }
}
...
void avdc_print_internals(avdark_cache_t *self)
{
    ...
    printf("----------------------------------\n");
    for (i = 0; i < (int)(self->number_of_sets*self->assoc); i++) {
        fprintf(stderr, "tag: <0x%.16lx> valid: %d timestamp: %ld\n",
            (long unsigned int)self->lines[i].tag,
            self->lines[i].valid,
            self->lines[i].timestamp);
        inc += 1;
        if (inc == (int)self->assoc){
            printf("----------------------------------\n");
            inc = 0;
        }
    }
